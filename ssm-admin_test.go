...
func TestPmmAdmin(t *testing.T) {
	...
	xVariables := map[string]string{
		"github.com/shatteredsilicon/ssm-client/ssm.Version": "gotest",
		"github.com/shatteredsilicon/ssm-client/ssm.RootDir": rootDir,
	}
	var ldflags []string
...
func testVersion(t *testing.T, data pmmAdminData) {
	...
	assert.Equal(t, "1.17.4", ssm.Version)
	expected := `gotest`
...
func testListNonEmpty(t *testing.T, data pmmAdminData) {
	...
	{
		dir, extension := ssm.GetServiceDirAndExtension()
...
`
	...
	t.Run("list --json", func(t *testing.T) {
		...
		expected := ssm.List{
			Version: "gotest",
			ServerInfo: ssm.ServerInfo{
				ClientName:        "test-client-name",
				ClientAddress:     "empty",
				ClientBindAddress: "(data)",
			},
			Services: []ssm.ServiceStatus{
				{
			...
			ExternalServices: []ssm.ExternalMetrics{},
		}
		got := ssm.List{}
...
EOF`)
	...
	f.Close()
	err = os.Chmod(filepath.Join(data.rootDir, ssm.AgentBaseDir, "bin/ssm-qan-agent-installer"), 0777)
	assert.NoError(t, err)
